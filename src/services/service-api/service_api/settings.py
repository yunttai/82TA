...
FAVORITE_WRITE_RATE_LIMIT_PER_MINUTE = int(
    os.environ.get("SERVICE_FAVORITE_WRITE_RATE_LIMIT_PER_MINUTE", "20")
)
...
if ENVIRONMENT == "production" and any(
    ...
    for limit in (
        ...
        FAVORITE_WRITE_RATE_LIMIT_PER_MINUTE,
...
FAVORITE_IDEMPOTENCY_DIGEST_KEY_VERSION = int(
    os.environ.get("SERVICE_FAVORITE_IDEMPOTENCY_DIGEST_KEY_VERSION", "1")
)
if FAVORITE_IDEMPOTENCY_DIGEST_KEY_VERSION != 1:
    raise RuntimeError(
        "SERVICE_FAVORITE_IDEMPOTENCY_DIGEST_KEY_VERSION must remain 1 until key-ring lookup is implemented"
    )
FAVORITE_IDEMPOTENCY_RETENTION_SECONDS = int(
    os.environ.get("SERVICE_FAVORITE_IDEMPOTENCY_RETENTION_SECONDS", "86400")
)
if FAVORITE_IDEMPOTENCY_RETENTION_SECONDS != 86_400:
    raise RuntimeError(
        "SERVICE_FAVORITE_IDEMPOTENCY_RETENTION_SECONDS must remain 86400 for Public 1.5"
    )
