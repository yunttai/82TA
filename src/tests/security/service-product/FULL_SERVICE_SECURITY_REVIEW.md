...
1. 실제 GCE에서 static IP → Nginx TLS → Web/Service containers 흐름, edge rate control, database backup/restore를 검증한 smoke evidence
2. 완료된 export를 사용자에게 전달하는 authenticated one-time download의 canonical 의미·구현과 실제 GCE private-volume worker/삭제 drill
...
Browser/PWA
  └─ HTTPS + same-origin cookie/CSRF
     GCE Nginx(CSP, no API cache, access-log redaction)
       └─ HTTPS-only custom domain
          └─ private Docker-network Service API
             ├─ encrypted Service PostgreSQL: identity, exact location, history, consent
             ├─ GCE private volume + Fernet: 15분 data export; scheduled worker/purge
...
- reverse-geocode의 정확한 좌표는 canonical GET query에 있으므로 로그 경계에서 차단했다. Gunicorn, Web Nginx와 GCE ingress Nginx의 API access log를 모두 끈다. 향후 Cloud Load Balancing/Cloud Armor를 추가하면 query·authorization·cookie가 수집되지 않는지 별도 evidence가 필요하다.
...
| 개인정보 log | PASS | Gunicorn/Web Nginx/GCE ingress Nginx 정적 배포 assertion |
...
| encrypted deployment | PARTIAL(static) | GCE Shielded VM, private/versioned GCS, non-root read-only containers 확인. managed DB/secret/backup evidence는 승격 전 필요 |
...
- 심각도: Medium
- flow: public route retry → 여러 Gunicorn/container worker → Routing 비용
- 영향: process-local idempotency/cache는 worker 간 replay를 보장하지 않아 중복 Routing call 또는 409가 발생할 수 있다.
- 현재 제어: owner-scoped key, request fingerprint, DB unique routing request ID, bounded TTL cache, GCE Nginx/application IP rate limits.
...
- 코드 상태: bounded worker와 management command가 PENDING job을 소비한다. export는 Fernet 암호화 파일로 기록되고 15분 TTL 후 물리 삭제된다. artifact 삭제 실패 시 DB reference를 유지하고 계정 hard-delete도 fail-closed한다. 현재 GCE 배포에는 이 lifecycle의 durable-volume/backup/alert evidence가 아직 없으므로 staging 승격 gate로 남는다.
...
- 유형: deployment gate
- 정적 제어: GCE static IP와 Nginx/Let's Encrypt HTTPS, CSP, private Routing network, host-side restricted secrets, ordered database/application startup. managed PostgreSQL/Redis와 backup/restore evidence는 승격 전 미검증이다.
